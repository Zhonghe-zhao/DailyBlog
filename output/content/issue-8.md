...
+++
title = " 6.s081学习"
date = "2025-05-28T12:51:30Z"
authors = ["Zhonghe-zhao"]
[taxonomies]
tags = [ "ComputerScience",  "OS", ]
[extra]
author = "Zhonghe-zhao"
avatar = "https://avatars.githubusercontent.com/u/147113943?v=4"
issue_url = "https://github.com/Zhonghe-zhao/DailyBlog/issues/8"
[extra.reactions]
thumbs_up = 0
thumbs_down = 0
laugh = 0
...
[[extra.comments]]
url = "https://github.com/Zhonghe-zhao/DailyBlog/issues/8#issuecomment-2952354527"
...
![Image](https://github.com/user-attachments/assets/f088e01c-c6ee-42bd-a05a-fd7b72da886c)

>**MMU**:是集成到CPU内的一个组件，主要是负责读取虚拟内存的映射表(**查看页表**)，并通过映射表将*虚拟地址* 转换为 *物理地址*。  
**RISC-V**指令可以操作虚拟地址(VA,virtual address)。


>从CPU的角度来说，一旦MMU打开了，它执行的每条指令中的地址都是虚拟内存地址。

>**satp**(Supervisor Address Translation and Protection Registers)：存放映射表的物理内存地址。由于映射表保存在内存中，*所以satp中保存的是**物理地址**可以让MMU找到内存中保存的映射表。例如在图例中VA = 0x1000，其中satp = 0x10的物理地址中保存的是这个VA的映射表。*


**字节（Byte）** 是内存中最小的可寻址单位，1 字节 = 8 位（bit）。

`char *p = malloc(4096);
`
向操作系统要了 **4096 字节** 的内存，地址存在 `p` 中。
...
p[1]      // 第 2 个字节（偏移 1）
...
p[4095]   // 第 4096 个字节（偏移 4095）
```


PTE ： PPN（物理页号）+ flag（权限）
...
PTE 中的 `PPN + flag` 其实就是：**这页该映射到哪里，能不能读写等**

页号决定你在哪页，offset 决定你在页内哪个字节


假设一个虚拟地址 VA = `0x00001234`

这个地址属于第几页？


`0x00001234 / 4096 = 1（整除）`
...
`offset = 0x00001234 % 4096 = 0x234（十进制是564）`


## 页里装的是什么？

页里装的是**你程序运行时真实用的数据**，比如：
...
|.data 段|已初始化的全局变量|
|.bss 段|未初始化的全局变量|
|堆（heap）|你 `malloc` 分配的内存|
|栈（stack）|函数调用产生的局部变量、返回地址等|
|mmap 区|映射的文件内容|
每一种内容会被放到不同的虚拟页里，这些虚拟页再映射到不同的物理页。 


假设你运行一个 C 程序，它调用了 `int *a = malloc(8192);`，即申请了 8KB 的堆内存：

- 操作系统会为你分配 **两页物理页**（2 × 4KB = 8192 字节）
    
- 虚拟地址可能是：`0x40000000 ~ 0x40001FFF`（2 页）
    
- 页表中会记录：这两个虚拟页 → 映射到两个物理页


 **如果没有“页”的概念，会怎样？**

那操作系统要**按字节**或者按更细粒度来管理内存，比如每次内存映射都搞成这样：

`0x40000000 → 0xA0000000   0x40000001 → 0xA0000001   0x40000002 → 0xA0000002`

结果就是：

- 页表会非常巨大（一个进程 4GB 虚拟地址 ➜ 就要 2³² 个条目）
...
~~~
int *a = malloc(8192); // 8KB
如果没有“页”的概念，系统得管理连续 8192 个字节的虚拟地址 ➜ 每个都映射到物理地址，太浪费了。
...
每一页内部，地址连续地被使用就好，不需要单独维护偏移映射
~~~



“页”让地址映射从**字节级 ➜ 页级**，大大降低了复杂度、提高了效率。  
没有页的概念，页表就会爆炸，系统根本跑不起来。页是虚拟内存机制的支柱。

## 理解为什么要用页管理

|虚拟地址|代表的含义|
|---|---|
|`0x00000000`|第 1 个字节（第 0 个地址）|
|`0x00000001`|第 2 个字节（第 1 个地址）|
|`...`|...|
|`0xFFFFFFFF`|第 2³² 个字节（最后一个地址）|

如果你按 **字节映射**

就表示你要为每个虚拟地址 **单独建立一个映射关系**：

```c
虚拟地址 0x00000000 ➜ 物理地址 X
虚拟地址 0x00000001 ➜ 物理地址 Y
...
虚拟地址 0xFFFFFFFF ➜ 物理地址 Z
```


所以你要存储 **2³² 条映射表项（PTE）**，一共就是 **4,294,967,296 条**！

每个页表项（PTE）如果是 8 字节（常见情况），  
那么你总共需要的页表大小是：
...
- 你只需要 2²⁰ 条页表项 = 1048576 条 PTE
    
- 乘上 8 字节 ➜ 总页表大小只需 8MB（可以接受）


**一条 PTE（页表项）** 管理一整页（4KB）内的所有虚拟地址，  
当程序访问某个地址时，用：  
`物理地址 = PPN × 页大小 + 页内偏移`
...
`char *p = malloc(1);  // 只要 1 字节`

但操作系统会为它分配一整页（4KB），这时：

- 实际只用到 **1 字节**
    
- 但分配了 **4096 字节**
    
- 剩下的 **4095 字节都闲着**，这就是**内部碎片**



## 问题

虚拟地址会存放数据吗？ 如果是的话那么虚拟地址中存放的数据不也会放在内存中的某些地方吗  


答：

✅ 你访问的是虚拟地址，但数据永远存放在**物理内存**的某个位置。虚拟地址只是**给你看的假象**，实际操作会被**MMU 转换成物理地址**再操作内存。
...
- 页表项（PTE）指向的是**一个真正的物理页**，比如某个用户的数据页、代码页。
    
- 它的权限位会设置 `PTE_R / PTE_W / PTE_X` 中的至少一个。
    
- 表示这就是最终的映射，**没有下一层了**。


## 什么是“非叶子页表项”（Middle PTE）

- 页表项指向的是**下一层页表的地址**，比如从 L2 → L1，或者 L1 → L0。
    
- 它**没有设置 RWX 权限**，只有 `PTE_V`。
    
- 它代表的是页表结构，还要继续往下走一层

'''
updated_at = "2025-06-07T10:58:26Z"
...
![Image](https://github.com/user-attachments/assets/7d19d7e4-f139-4902-8975-65a70cc2d956)
...
![Image](https://github.com/user-attachments/assets/3963fdd5-c8be-4808-b291-bbb7af5afb9e)
...
![Image](https://github.com/user-attachments/assets/41bf1ee9-618f-491b-beaa-afcdbbc1de90)
...
![Image](https://github.com/user-attachments/assets/88b08622-7de2-4d0b-90d2-7ba6c921fc46)
...
![Image](https://github.com/user-attachments/assets/8d087a24-c559-4809-9e9d-3a499cd1a26a)


使用 `gdb-multiarch` 指令开启gdb 并且使用 `target remote :25000` 连接到qemu


![Image](https://github.com/user-attachments/assets/53393104-daef-49a8-a763-a9be02dff4f7)
...
![Image](https://github.com/user-attachments/assets/6aa9c086-a65d-49bb-95e1-2cb0216dcc13)
...
![Image](https://github.com/user-attachments/assets/f9fa6347-2430-4f65-878a-9dbbe4227cec)
...
![Image](https://github.com/user-attachments/assets/e6c58e1d-b24b-4713-8019-50ec8a481734)
...
![Image](https://github.com/user-attachments/assets/2fe3d767-68c6-4c98-ab8a-440f8defc461)



如何计算

![Image](https://github.com/user-attachments/assets/dcbd1dde-7309-49b4-a3ea-41431177ee2d)
...
`typedef uint64* pagetable_t;  // 页表是 uint64 的指针，指向 512 个 PTE（页表项）
`
`pagetable_t pagetable;  // 等价于：uint64* pagetable;
`
`pagetable[i] 等价于 *(pagetable + i)`
